...
function Page() {
  const params = useParams<{ username: string; linkSlug: string }>();
  const username = decodeURIComponent(params.username ?? '');
  const customLinkSlug = decodeURIComponent(params.linkSlug ?? '').trim().toLowerCase();

  const displayUsername = username || 'this user';
  const displayProductName = useMemo(() => {
    if (!customLinkSlug) return 'this product';
    return customLinkSlug.replace(/-/g, ' ');
  }, [customLinkSlug]);
  ...
  async function handleSendMessage() {
    if (!username || !customLinkSlug) {
      setSendStatus({ type: 'error', text: 'Invalid product link URL.' });
      return;
    }
